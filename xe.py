...
# --- Основные вкладки ---
main_tab1, main_tab2 = st.tabs(["🧮 Основной расчёт", "⚖️ Расчёт по продукту"])
...
# --- Вкладка 2: расчёт по продукту ---
with main_tab2:
    st.subheader("⚖️ Расчёт ХЕ по продукту")
    st.write("Введите БЖУ на 100 г продукта и массу продукта:")
    ...
    if st.button("Рассчитать ХЕ для продукта", type="primary", use_container_width=True):
...
# --- Вкладки со справкой и историей ---
tab1, tab2 = st.tabs(["📘 Как компенсировать БЖЕ?", "📜 История расчётов"])

with tab1:
    st.markdown("""
    ### 💉 Как компенсировать БЖЕ?
    1 БЖЕ = 1 ХЕ. Дозу нужно растягивать или подкалывать через 1–2–3 ч после еды.

    #### ⏳ Как растягивать:
    • 1 БЖЕ — на 3 ч  
    • 2 БЖЕ — на 4 ч  
    • 3 БЖЕ — на 5 ч  
    • Более 4 БЖЕ — от 5 до 10+ ч  
